...
> **Provenance and tier.** Salvaged from the retired `powerautomate-flow` skill. The operation
> shapes are `PLATFORM`; the specific list names, site URLs and column names in the examples are
> `EXAMPLE` from one system — replace them.
>
> **Unverified against an import.** These recipes were written before a real export existed here.
> Treat an operation you have not seen in a working flow as a candidate: build it, import it, and
> if it works, record it via `prompts/feedback-session.md` so the next person does not re-test it.
...
3. Office 365 Users — `shared_office365users`
4. Approvals — `shared_approvals`
5. Microsoft Dataverse — `shared_commondataserviceforapps`
6. Teams — `shared_teams`
7. Notifications (mobile) — `shared_flowpush`
8. HTTP — no connector
9. Data Operations — no connector
10. Checklist when adding a connector to a flow
